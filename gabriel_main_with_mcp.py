"""
uv run --with nltk --with anthropic --with smithery --with mcp gabriel_main_with_mcp.py
"""
...
agent = MCPAgent(model_name="claude-3-7-sonnet-20250219", temperature=1, system_prompt=SYSTEM_PROMPT) 
...
async def main():
    ...
    # Use async_reset instead of reset
    ...
    while not done:
        ...
        # Use async_get_observation instead of get_observation
        ...
        action = await agent(observation_str)
        ...
        # Use async_step if available, otherwise we'll need to convert this too
        if hasattr(env, 'async_step'):
            done, info = await env.async_step(action=action)
        else:
    ...
    # Use async_close if available
    if hasattr(env, 'async_close'):
